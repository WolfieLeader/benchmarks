...
func (c *Client) Run(endpoint *Endpoint, timeout time.Duration) (time.Duration, bool) {
	if endpoint == nil || endpoint.Path == "" || endpoint.Method == "" || endpoint.Expected == nil || endpoint.Expected.Body == nil || endpoint.Expected.StatusCode < 100 || endpoint.Expected.StatusCode > 599 {
		return 0, false
	}
	...
	if !jsonContains(endpoint.Expected.Body, actualBody) {
		return latency, false
	}
