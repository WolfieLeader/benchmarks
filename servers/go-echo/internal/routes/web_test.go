...
// webEcho mirrors the production echo wiring (json/v2 serializer) so c.JSON and
// utils.BindJSON behave in tests exactly as they do under app.New().
func webEcho() *echo.Echo {
	e := echo.New()
	e.JSONSerializer = utils.JSONSerializer{}
	RegisterWeb(e, testSecret)
	return e
}

func do(t *testing.T, e *echo.Echo, req *http.Request) (*httptest.ResponseRecorder, string) {
	t.Helper()
...
func TestHTML(t *testing.T) {
	rec, body := do(t, webEcho(), httptest.NewRequest(http.MethodGet, "/html", nil))
...
func TestCompute(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			rec, body := do(t, e, httptest.NewRequest(http.MethodGet, "/compute?"+tc.query, nil))
...
func TestJWTSignThenVerify(t *testing.T) {
	...
	rec, body := do(t, e, httptest.NewRequest(http.MethodGet, "/jwt/sign", nil))
	if rec.Code != http.StatusOK {
