...
| Zig                    | **One server** (http.zig), **all 4 databases**, no shared layer (single implementation). A `shared/zig` layer is extracted only when a **second Zig consumer** appears — same multi-consumer trigger as Python's async repos (§3).                                                                                                                                                                                                 |
...
- **oha 1.14.0 via `brew install oha`** — the calibration gate's reference generator (`just calibrate`, docs/calibration.md). Chosen because it is the only maintained tool with wrk2-style intended-send-time latency correction (`-q <rate> --latency-correction`) plus JSON output; needed only when re-calibrating, never during benchmark runs.
...
- **Zig 0.16.0 via `brew install zig`** — brew is exactly at 0.16.0 (no lag). Pulls llvm@21/lld@21 as deps. The Zig server `@cImport`s two C database drivers, so **local builds also need `brew install mongo-c-driver cassandra-cpp-driver pkgconf`**. Driver-naming differs by platform and `build.zig` bridges it with a pkg-config probe: Homebrew ships **mongo-c-driver 2.x** (pkg-config module `mongoc2`, which bundles libbson), while Alpine's `mongo-c-driver-dev` is the **1.27.x** line exposing the legacy `libmongoc-1.0`/`libbson-1.0` pair — `build.zig` tries `mongoc2` first and falls back to the dual `-1.0` modules so one build file works on both. The Cassandra cpp-driver ships **no** pkg-config file on Homebrew, so on macOS `build.zig` adds `/opt/homebrew/{include,lib}` explicitly; Alpine's `cassandra-cpp-driver-dev` is found via pkg-config normally. (Runtime image needs only the non-`-dev` `mongo-c-driver`/`cassandra-cpp-driver` + `libstdc++`/`libgcc`.)
...
- **Already correct**: Bun 1.3.14, uv 0.11.25, pnpm 11.9.0, just 1.54.0, Docker 29.3.0.
...
| ---------- | --------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| TypeScript | **pnpm workspace** (the only workspace) | The one language where a workspace earns its keep: shared's runtime deps (drizzle/mongodb/ioredis/cassandra-driver) install once and resolve via `workspace:*`; `file:`/`link:` would re-create version-skew duplication. Bun consumes pnpm-installed `node_modules` fine. **Deno does not read `pnpm-workspace.yaml`** — root `package.json` `workspaces` is the Deno workspace mirror (produces `deno.lock`), with `nodeModulesDir: "manual"` in the member `deno.json` (spiked in 0B; no root `deno.json` shipped). **0D acceptance: shared TS code must be proven consumed from all three runtimes (node/bun/deno) via the contract gate before the lane closes.** |
...
- **Redis teardown canon**: graceful shutdown across the TS servers closes Redis with an immediate `disconnect()` (not `quit()`) — benchmark teardown wants a prompt socket close, not a drain-and-wait for in-flight replies. `disconnect()` is the standard; the deno-oak server's earlier `quit()` was drift, corrected to match — **ruled 2026-07-04**.
- **Build — `@bench/shared` (and `@bench/hono-app`) are modular `src/` bundled to ONE artifact by `tsdown` with oxc-generated declarations.** Each package's `src/` is split into coherent modules (shared: `consts`/`env`/`schemas`/`id`/`db-types`/`db-{postgres,mongodb,redis,cassandra}`/`db-registry`; hono-app: `app`/`db-routes`/`params-routes`) with `index.ts` as the public barrel; internal imports use explicit `./module.ts` specifiers (`allowImportingTsExtensions`, `noEmit` — `tsc` is now typecheck-only, `tsc --noEmit`). `tsdown@0.22.3` (rolldown) bundles the modular src into **one self-contained `dist/index.js` + one `dist/index.d.ts`** — cross-file types inlined, the real runtime deps (DB drivers/zod; `hono/*` + `@bench/shared` for hono-app) kept **external** as preserved imports — consumed **identically** by every runtime (Node/Bun/Deno) via the package's single `.` export. NestJS still emits to `dist/` via `tsc`; the other server apps typecheck `--noEmit` and run via their runtime. **The declaration build uses `dts: { oxc: true }` (oxc isolated-declarations), NOT `dts: true`:** oxc's isolated-declarations emitter needs no TypeScript programmatic API, so it runs under the TS 7.0.1-rc — whereas `dts: true` (the TS-compiler-backed mode) **FAILS on the RC** because TS 7.0.1-rc ships **no stable JS compiler API** (§0.1; typescript.md rule 40). This is the lead-proven escape hatch that made a modular layout viable ahead of TS 7.1. The old cross-runtime `.d.ts` trap (a multi-file `tsc`-emitted `.d.ts` tree resolves inconsistently — NodeNext wants `.js` specifiers, Deno resolves those to runtime files and drops type-only members) is sidestepped the same way single-file did: the bundle is one self-contained artifact with no cross-file re-exports. oxc isolated-declarations requires an explicit type annotation on every export (added as annotations only, no logic change). **Revisit `dts: true` (full TS-compiler dts mode) when TS 7.1 restores the programmatic API.**
...
**Discovery — make the client folder-structure aware.** Today the roster lives hardcoded in `config/config.json` (`servers` list) and drifts from reality. Instead: each server app carries a small manifest (`bench.json`) next to its Dockerfile declaring `{name, language, runtime, image, port, databases, experimental, dev_port}` (the shipped shape — `bench.json` carries both the container `port` and the `dev_port`; the scripts require at minimum `name`/`runtime`/`image`/`port`; the Go client's roster requires only `name`/`image`/`port` (it never reads `runtime`)). The client **discovers the roster by scanning `servers/*/bench.json`** (one-level walk over the flat layout, §2.1) — adding a server = adding a folder, zero central edits. `config/config.json` keeps only benchmark parameters (suites, endpoints, load profiles, container limits); the schema validates both.
...
| `just update` | run across all stacks as part of Phase 0; extend it to also cover the lint/format tooling above. **Must be pin-aware**: blanket "update to latest" resolves the `latest` dist-tag and would **clobber deliberate prerelease pins** (`typescript@7.0.1-rc` — `latest` is still 6.0.3; Go 1.27rc1; drizzle 1.0-rc) and **lockstep sets** (elysia + `@elysiajs/node`). Pinned/prerelease deps are exempt from auto-bump and tracked in a short "pinned deps" list in the README.                                                         |
...
4. Only after move-only extraction is stable, do behavior-affecting swaps as separate slices: TS `pg` → `postgres`, TS 7 RC typecheck gate, the shared package build (now modular `src/` bundled by tsdown with `dts: { oxc: true }` — the lead-proven RC-safe escape hatch; full `dts: true` deferred to TS 7.1 — see §4), FastAPI worker/pool normalization, Bun shutdown fix.
...
2. **Root sharing files are the hidden contention.** With §2.2's per-language path deps (go.mod `replace`, uv sources, cargo paths are all per-server files — no contention), the only shared root files left are `pnpm-workspace.yaml` + the mirrored `workspaces` array in root `package.json` (TS, read by Deno) and `settings.gradle` (Kotlin). Two lanes each appending a member line there conflict trivially. Serialize those one-line edits (or accept the trivial conflict) — they are not a reason to serialize the lanes themselves.
